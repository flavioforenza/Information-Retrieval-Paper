    ...
    \begin{minipage}{\linewidth}
        ...
        \begin{minipage}{0.45\linewidth}
            \begin{block}{\centering Laplace Smoothing}
                \small $$ P(w_i|w_{i-1}) = \frac{count(w_{i-1})+1}{count(w_{i-1})+|V|} $$
            \end{block}
        \end{minipage}
        \hspace{0.05\linewidth}
        \begin{minipage}{0.45\linewidth}
            \begin{block}{\centering Linear Interpolation Smoothing (Bi-grams)}
                \scriptsize $$ P(w_i|w_{i-1}) = \lambda P(q|M_d) + (1-\lambda)P(q|M_c) $$
            \end{block}
        \end{minipage}
    \end{minipage}
    \begin{minipage}{\linewidth}
        ...
        \begin{minipage}{0.45\linewidth}
            ...
            \begin{block}{\centering Linear Interpolation Smoothing (Zero-grams)\footnotemark[3]}
                \small $$ P(w_i) = \lambda \frac{1}{|V|} + (1-\lambda)P(w_i) $$
            \end{block}
        \end{minipage}
    \end{minipage}
...
\begin{frame}{POSITIVE POINTWISE MUTUAL INFORMATIONS (PPMI)}
    To evaluate the relationship between two words we can use the Pointwise 
    Mutual Informations (PMI).
    \begin{block}{PPMI}
        \small \emph{\say{PPMI draws on the intuition that the best way to weigh the association between two words is to ask how much more the two words co-occur in our corpus than we would have a priori expected them to appear by chance.}\footnotemark[2]}
    \end{block}
    ...
    \begin{minipage}{\linewidth}
        ...
        \begin{minipage}{0.40\linewidth}
            \begin{block}{\centering PMI}
                \small $$ PMI(x,y) = \log_2\frac{P(x,y)}{P(x)P(y)} $$
            \end{block} 
        \end{minipage}
        \hspace{0.05\linewidth}
        \begin{minipage}{0.50\linewidth}
            \begin{block}{\centering PPMI}
                \small $$ PPMI(x,y) = \max(\log_2\frac{P(x,y)}{P(x)P(y)}, 0) $$
            \end{block}
        \end{minipage}
    \end{minipage}
\end{frame}
